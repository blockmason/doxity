...
exports.default = function (src) {
  ...
  return new Promise(function (resolve) {
      if (false)//(_fs2.default.existsSync(process.env.PWD + '/truffle.js'))
      {
        ...
        resolve({
          contracts: Object.keys(res).reduce(function (o, k) {
            var _res$k$rawData = res[k].rawData,
                ...
                rest = _objectWithoutProperties(_res$k$rawData, ['metadata']);

            var _JSON$parse = JSON.parse(metadata),
                output = _JSON$parse.output,
                settings = _JSON$parse.settings;

            var fN = Object.keys(settings.compilationTarget)[0];
            var fileName = fN.indexOf(process.env.PWD) === 0 ? fN : process.env.PWD + '/node_modules/' + fN;
            return _extends({}, o, _defineProperty({}, k, _extends({}, rest, output, { fileName: fileName })));
          }, {})
        });
      });
    } else {
